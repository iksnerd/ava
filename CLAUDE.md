...
below, and `ava monitor`, the realtime call-transcript monitor in
`internal/monitor`, see `docs/monitor.md`), `mlx-engine/` (Python Kokoro TTS server used by `ava`),
...
Voxtral is still the default engine of `ava monitor`, which is a different
...
- `internal/monitor/` - `ava monitor`, the realtime transcript monitor (wired in by `cmd/ava/monitor.go`): serves a live transcript over SSE at localhost and logs it to a file; input can be the mic or a loopback device (e.g. BlackHole) for capturing call audio
...
- `pkg/stt/realtime/` - a *different*, independent client from `pkg/mlx`: wraps `voxtral/realtime.py` directly via `os/exec`, used only by `internal/monitor`. Streaming Voxtral, where `mlx-engine` serves only Kokoro TTS.
- `mlx-engine/` - local Kokoro TTS server for `ava` (Python, `uv`-managed — see `mlx-engine/README.md`)
- `voxtral/` - Python/MLX primitives for `ava monitor` (uv project): Voxtral STT (Mini 4B Realtime), a Whisper fallback engine (multilingual, for languages Voxtral doesn't cover), and Sortformer speaker diarization; see `make setup-voxtral` and `docs/monitor.md`
...
- **Minimal Go dependencies** - two third-party Go dependencies, both load-bearing: `github.com/spf13/cobra` (CLI command/flag framework), for `ava`'s command tree, and `github.com/modelcontextprotocol/go-sdk` (used only by `cmd/ava/mcp.go`) — hand-rolling JSON-RPC-over-stdio against a spec that still moves would cost more than the dependency does. Don't add a third without the same kind of reason. `voxtral/` is external Python/MLX tooling shelled out to via `os/exec`, same as whisper-cli/sox — not a Go dependency. (The Python/Swift components have their own dependency managers — `uv` and SwiftPM respectively.) There is one binary, `ava`. Its cobra wiring lives in `cmd/ava/`: `main.go` just calls `Execute()`, `root.go` builds the root command, and each subcommand (e.g. `engine.go`, `monitor.go`) is its own file with a `newXCmd()` constructor. A subcommand big enough to be its own subsystem keeps its code in an `internal/` package and its file here is the wiring, as `monitor.go` does for `internal/monitor`.
