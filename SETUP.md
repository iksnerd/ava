...
## Done
- [x] `pkg/voxtral` (Go wrapper around `realtime.py`) + `voxtral/` (MLX primitives: `stt.py`, `realtime.py` — `stt.py` is a one-shot script run directly, not wrapped in Go; nothing here calls it through `pkg/voxtral`), managed as a uv project (`voxtral/pyproject.toml` + `uv.lock`) — `make setup-voxtral` runs `uv sync` into `voxtral/.venv`
