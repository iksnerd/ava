...
- The permission belongs to the **parent process**, not to `ava`. The
...
If you passed a file to `ava transcribe` and got this, the file is
...
`ava --no-paste` to skip the attempt entirely.
...
```bash
ava setup-model               # or: make setup-model, from a checkout
ava setup-model --model tiny  # if you run with --model tiny
```
...
That means the Kokoro server is down and `say` took over. `ava speak`
...
`ava voices` shows the configured voice, so a wrong-sounding voice is
visible there too.
